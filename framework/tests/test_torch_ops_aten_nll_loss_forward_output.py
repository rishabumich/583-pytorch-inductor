...
class Torch_Ops_Aten_NllLossForward_OutputModule(torch.nn.Module):
    def forward(self, x, target, weight, reduction, ignore_index, output, total_weight):
        return torch.ops.aten.nll_loss_forward.output(x, target, weight, reduction, ignore_index, output, total_weight)

mod = Torch_Ops_Aten_NllLossForward_OutputModule()
