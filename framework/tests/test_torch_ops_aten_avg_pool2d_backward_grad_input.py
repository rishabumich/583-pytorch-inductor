...
class Torch_Ops_Aten_AvgPool2DBackward_GradInputModule(torch.nn.Module):
    def forward(self, grad_output, x, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override, grad_input):
        return torch.ops.aten.avg_pool2d_backward.grad_input(grad_output, x, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override, grad_input)

mod = Torch_Ops_Aten_AvgPool2DBackward_GradInputModule()
