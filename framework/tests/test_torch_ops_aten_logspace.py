...
class Torch_Ops_Aten_LogspaceModule(torch.nn.Module):
    def forward(self, start, end, steps, base, dtype, layout, device, pin_memory):
        return torch.ops.aten.logspace(start, end, steps, base, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_LogspaceModule()
