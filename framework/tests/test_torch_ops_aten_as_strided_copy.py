...
class Torch_Ops_Aten_AsStridedCopyModule(torch.nn.Module):
    def forward(self, x, size, stride, storage_offset):
        return torch.ops.aten.as_strided_copy(x, size, stride, storage_offset)

mod = Torch_Ops_Aten_AsStridedCopyModule()
