...
class Torch_Ops_Aten_NllLossBackwardModule(torch.nn.Module):
    def forward(self, grad_output, x, target, weight, reduction, ignore_index, total_weight):
        return torch.ops.aten.nll_loss_backward(grad_output, x, target, weight, reduction, ignore_index, total_weight)

mod = Torch_Ops_Aten_NllLossBackwardModule()
