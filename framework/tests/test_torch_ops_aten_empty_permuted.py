...
class Torch_Ops_Aten_EmptyPermutedModule(torch.nn.Module):
    def forward(self, size, physical_layout, dtype, layout, device, pin_memory):
        return torch.ops.aten.empty_permuted(size, physical_layout, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_EmptyPermutedModule()
...
args = (size, physical_layout, dtype, layout, device, pin_memory,)
