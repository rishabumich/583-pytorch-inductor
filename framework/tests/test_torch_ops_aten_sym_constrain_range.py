import torch
from torch.export import export_for_training
from torch._decomp import decomposition_table

class Torch_Ops_Aten_SymConstrainRangeModule(torch.nn.Module):
    def forward(self, size, min, max):
        return torch.ops.aten.sym_constrain_range(size, min, max)

mod = Torch_Ops_Aten_SymConstrainRangeModule()

size = torch.tensor(0)  # Fallback for unknown type |Scalar
min = torch.tensor(0)  # Fallback for unknown type int?
max = torch.tensor(0)  # Fallback for unknown type int?

args = (size, min, max,)

ep = export_for_training(mod, args)
print("Before decomposition:")
print(ep.module().code)

ep = ep.run_decompositions(decomposition_table)
print("After decomposition:")
print(ep.module().code)
