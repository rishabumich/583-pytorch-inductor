...
class Torch_Ops_Aten_OnesLikeModule(torch.nn.Module):
    def forward(self, x, dtype, layout, device, pin_memory, memory_format):
        return torch.ops.aten.ones_like(x, dtype, layout, device, pin_memory, memory_format)

mod = Torch_Ops_Aten_OnesLikeModule()
