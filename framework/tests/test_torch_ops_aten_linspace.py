...
class Torch_Ops_Aten_LinspaceModule(torch.nn.Module):
    def forward(self, start, end, steps, dtype, layout, device, pin_memory):
        return torch.ops.aten.linspace(start, end, steps, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_LinspaceModule()
