...
class Torch_Ops_Aten_Randn_GeneratorModule(torch.nn.Module):
    def forward(self, size, generator, dtype, layout, device, pin_memory):
        return torch.ops.aten.randn.generator(size, generator, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_Randn_GeneratorModule()
