...
class Torch_Ops_Aten_FftIhfftn_OutModule(torch.nn.Module):
    def forward(self, x, s, dim, norm, out):
        return torch.ops.aten.fft_ihfftn.out(x, s, dim, norm, out=out)

mod = Torch_Ops_Aten_FftIhfftn_OutModule()
