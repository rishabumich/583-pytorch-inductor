...
class Torch_Ops_Aten_Rand_GeneratorWithNamesModule(torch.nn.Module):
    def forward(self, size, generator, names, dtype, layout, device, pin_memory):
        return torch.ops.aten.rand.generator_with_names(size, generator, names, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_Rand_GeneratorWithNamesModule()
