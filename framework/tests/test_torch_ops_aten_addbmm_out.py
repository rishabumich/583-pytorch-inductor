...
class Torch_Ops_Aten_Addbmm_OutModule(torch.nn.Module):
    def forward(self, x, batch1, batch2, beta, alpha, out):
        return torch.ops.aten.addbmm.out(x, batch1, batch2, beta, alpha, out=out)

mod = Torch_Ops_Aten_Addbmm_OutModule()
