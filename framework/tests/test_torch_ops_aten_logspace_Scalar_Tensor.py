...
class Torch_Ops_Aten_Logspace_ScalarTensorModule(torch.nn.Module):
    def forward(self, start, end, steps, base, dtype, layout, device, pin_memory):
        return torch.ops.aten.logspace.Scalar_Tensor(start, end, steps, base, dtype, layout, device, pin_memory)
