...
class Torch_Ops_Aten_Sort_ValuesStableModule(torch.nn.Module):
    def forward(self, x, stable, dim, descending, values, indices):
        return torch.ops.aten.sort.values_stable(x, stable, dim, descending, values, indices)

mod = Torch_Ops_Aten_Sort_ValuesStableModule()
