...
class Torch_Ops_Aten_TrilIndicesModule(torch.nn.Module):
    def forward(self, row, col, offset, dtype, layout, device, pin_memory):
        return torch.ops.aten.tril_indices(row, col, offset, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_TrilIndicesModule()
