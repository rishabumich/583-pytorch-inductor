...
class Torch_Ops_Aten_Linspace_TensorTensorModule(torch.nn.Module):
    def forward(self, start, end, steps, dtype, layout, device, pin_memory):
        return torch.ops.aten.linspace.Tensor_Tensor(start, end, steps, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_Linspace_TensorTensorModule()
