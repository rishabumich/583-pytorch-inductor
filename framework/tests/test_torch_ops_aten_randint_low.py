...
class Torch_Ops_Aten_Randint_LowModule(torch.nn.Module):
    def forward(self, low, high, size, dtype, layout, device, pin_memory):
        return torch.ops.aten.randint.low(low, high, size, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_Randint_LowModule()
...
args = (low, high, size, dtype, layout, device, pin_memory,)
