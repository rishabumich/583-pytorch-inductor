...
class Torch_Ops_Aten_QuantizedLstm_InputModule(torch.nn.Module):
    def forward(self, input, hx, params, has_biases, num_layers, dropout, train, bidirectional, batch_first, dtype, use_dynamic):
        return torch.ops.aten.quantized_lstm.input(input, hx, params, has_biases, num_layers, dropout, train, bidirectional, batch_first, dtype, use_dynamic)

mod = Torch_Ops_Aten_QuantizedLstm_InputModule()
