...
class Torch_Ops_Aten_EmptyStridedModule(torch.nn.Module):
    def forward(self, size, stride, dtype, layout, device, pin_memory):
        return torch.ops.aten.empty_strided(size, stride, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_EmptyStridedModule()
...
args = (size, stride, dtype, layout, device, pin_memory,)
