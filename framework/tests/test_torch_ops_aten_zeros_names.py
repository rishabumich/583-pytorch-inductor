...
class Torch_Ops_Aten_Zeros_NamesModule(torch.nn.Module):
    def forward(self, size, names, dtype, layout, device, pin_memory):
        return torch.ops.aten.zeros.names(size, names, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_Zeros_NamesModule()
