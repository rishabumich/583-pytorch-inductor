...
class Torch_Ops_Aten_NewOnesModule(torch.nn.Module):
    def forward(self, x, size, dtype, layout, device, pin_memory):
        return torch.ops.aten.new_ones(x, size, dtype, layout, device, pin_memory)

mod = Torch_Ops_Aten_NewOnesModule()
